...
export default function Home() {
	...
	const [modalIsOpen, setModalIsOpen] = useState(false);	
		...
		>(null);
			...
			const zoomInActive = (step = 0.3, max = 5) => {
			...
			const zoomOutActive = (step = 0.3, min = 1) => {
				...
				setModalZoomes((prev) => {
					...
					// if zoom returned to 1, reset offset
					if (next <= 1) setModalOffsets((o) => ({ ...o, [idx]: { x: 0, y: 0 } }));
					return { ...prev, [idx]: next };
				});
			};

			// Pointer (mouse/touch) drag handlers for panning
  ...
  // Pagination state for Koleksi Parfume
	const parfumRows = [
		...
		[
			{ name: "Amyris Femme", images: ["/file.svg"], price: "$295", desc: "Aroma floral lembut dan feminin." },
			{ name: "Gentle Fluidity Gold", images: ["/globe.svg"], price: "$310", desc: "Wangi musky vanilla yang elegan." },
			{ name: "L'Homme À la rose", images: ["/LogoGP1.png"], price: "$280", desc: "Maskulin segar dengan sentuhan rose." },
		],
		[
			{ name: "Aqua Vitae", images: ["/vercel.svg"], price: "$250", desc: "Citrus segar, cocok untuk siang hari." },
			{ name: "Oud", images: ["/window.svg"], price: "$390", desc: "Oud klasik, mewah dan tahan lama." },
			{ name: "Petit Matin", images: ["/file.svg"], price: "$265", desc: "Aroma pagi Paris yang lembut." },
		],
	];
						...
						return (
							<div className={`relative w-full max-w-xs sm:max-w-[320px] aspect-square mb-3 sm:mb-4 overflow-hidden mx-auto ${isRound ? 'rounded-3xl' : 'rounded-xl'}`}>
								<Image
									...
									alt={product.name}
									fill
									className={`${isRound ? 'object-cover rounded-3xl' : 'object-cover rounded-xl'} group-hover:scale-105 transition-transform duration-300`}
								...
								<div className={`absolute inset-0 flex items-center justify-center bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${isRound ? 'rounded-3xl' : 'rounded-xl'}`}>
									...
									<div className={`${product.name.includes('FRUIROSA') ? 'text-2xl sm:text-3xl font-extrabold' : 'text-base sm:text-lg font-semibold'} mb-1 text-black text-center`}>
										{product.name.includes('Eau De Parfum') ? (
											<>
												<span className="block">{product.name.replace(/\s*Eau De Parfum$/i, '')}</span>
												<span className="block text-sm font-medium text-gray-600">Eau De Parfum</span>
											</>
										) : (
											product.name
										)}
