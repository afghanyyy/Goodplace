...
export default function MobileSmartSearch({ openProduct, products }: MobileSmartSearchProps) {
  ...
  const { setHidden } = useFloatingButtons();
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((key.toLowerCase() === 'a' || code === 'KeyA') && (e.ctrlKey || e.metaKey)) {
        // If input is focused, select input text
        const active = document.activeElement;
        if (inputRef.current && (active === inputRef.current || inputRef.current.contains(active))) {
          (inputRef.current as HTMLInputElement).select();
        } else if (resultsRef.current) {
          // Select the results container content
              ...
              <div className="px-4 py-3 border-b border-black/5 flex items-center gap-3">
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full bg-transparent border-none outline-none text-black/90 placeholder-black/40 text-base select-text"
                  />
                </div>
                {searchQuery && (
                  <motion.button
